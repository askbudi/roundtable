...
# Increased buffer limit for subprocess stdout/stderr to handle large outputs
# Default asyncio limit is 64KB (2^16), we increase to 10MB to handle large responses
SUBPROCESS_STREAM_LIMIT = 10 * 1024 * 1024  # 10MB


async def create_subprocess_with_large_buffer(*args, limit=SUBPROCESS_STREAM_LIMIT, **kwargs):
    """Create a subprocess with increased stream buffer limit.
    
    This helper function creates a subprocess using asyncio.create_subprocess_exec
    with a custom stream limit to prevent LimitOverrunError when reading large outputs.
    
    The default asyncio StreamReader limit is 64KB (2^16 bytes), which can cause
    errors like "Separator is found, but chunk is longer than limit" when subagents
    return large responses (especially with tool outputs, file contents, etc).
    
    Args:
        *args: Positional arguments for create_subprocess_exec
        limit: Stream buffer limit in bytes (default: 10MB)
        **kwargs: Keyword arguments for create_subprocess_exec
        
    Returns:
        asyncio.subprocess.Process with increased stream buffer limits
    ...
    # Note: We can't directly set the limit on create_subprocess_exec
    # But we can create a custom protocol that sets the limit
    
    # Use a custom stream reader factory with increased limit
    class _LargeBufferSubprocessProtocol(asyncio.SubprocessProtocol):
        """Custom protocol that creates stream readers with larger buffers."""
        ...
        def connection_made(self, transport):
            ...
            # Create streams with increased limit
            if transport.get_pipe_transport(1):  # stdout
                self.stdout = asyncio.StreamReader(limit=self.limit, loop=self.loop)
                transport.get_pipe_transport(1)._protocol.set_transport(
                    transport.get_pipe_transport(1)
                )
                
            if transport.get_pipe_transport(2):  # stderr
                self.stderr = asyncio.StreamReader(limit=self.limit, loop=self.loop)
                transport.get_pipe_transport(2)._protocol.set_transport(
                    transport.get_pipe_transport(2)
                )
        ...
        def process_exited(self):
            for waiter in self._exit_waiters:
                if not waiter.done():
                    waiter.set_result(self.transport.get_returncode())
                    
    # Actually, the above approach is too complex and won't work with asyncio's API.
    # The simpler approach is to just use create_subprocess_exec and handle large
    # outputs by reading in chunks instead of lines, or by catching LimitOverrunError.
    # However, Python 3.13+ allows setting limit via environment variable or
    # we can monkey-patch the StreamReader class.
    
    # For now, let's use a simpler approach: wrap create_subprocess_exec and
    # set the limit on the stream readers after creation.
    # This is a bit hacky but works.
    
    # Actually, the cleanest solution is to use create_subprocess_shell/exec
    # and then replace the stream readers with ones that have a higher limit.
    # But this requires accessing internal APIs.
    
    # The most robust solution is to patch asyncio's default limit before creating
    # the subprocess, then restore it. But this has race conditions.
    
    # Let's use a different approach: create the subprocess normally, but configure
    # the stream protocol to use a larger limit. Since we can't do this directly,
    # we'll need to use the low-level API.
    
    import sys
    loop = asyncio.get_event_loop()
    
    # For Python 3.13+, we can pass limit parameter
    # For earlier versions, we need to use a workaround
    if sys.version_info >= (3, 13):
        # Python 3.13+ supports limit parameter in create_subprocess_exec
        try:
    ...
    # Workaround for Python < 3.13: create process and monkey-patch the streams
    process = await asyncio.create_subprocess_exec(*args, **kwargs)
    
    # Try to increase the limit on existing stream readers
    if hasattr(process.stdout, '_limit'):
        process.stdout._limit = limit
    if hasattr(process.stderr, '_limit'):
        process.stderr._limit = limit
...
class AdapterSession:
    """Async context manager for adapter lifecycle management.

    Ensures proper setup and teardown of subprocess/client resources.
    Inspired by TinyAgent's MCP client pattern.
    """

    def __init__(self, client, start_method=None, stop_method=None):
        """Initialize session with client and lifecycle methods.

        Args:
            client: The client object to manage (e.g., _ACPClient)
            start_method: Async method to call on enter (default: client.start)
            stop_method: Async method to call on exit (default: client.stop)
        """
        self.client = client
        self.start_method = start_method or (client.start if hasattr(client, 'start') else None)
        self.stop_method = stop_method or (client.stop if hasattr(client, 'stop') else None)
        self.started = False

    async def __aenter__(self):
        """Start the client/process and register handlers."""
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        """Clean up: unregister handlers, cancel tasks, terminate process."""
        if self.started and self.stop_method:
            try:
...
@asynccontextmanager
async def adapter_session(client, start_method=None, stop_method=None):
    """Create an async context manager for adapter lifecycle.

    Usage:
        async with adapter_session(client) as c:
            result = await c.request("method", params)
    """
    session = AdapterSession(client, start_method, stop_method)
    try:
